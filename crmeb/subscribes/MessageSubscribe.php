...
class MessageSubscribe
{
    ...
    {
        if($switch && $phone){
            $res = AliSMS::sendCode($phone, $data ,$template);
    ...
    {	
        ...
        $this->send($switch,$storeInfo->user_phone,compact('order_id','pay_price'),AliSMS::PAY_SUCCESS_CODE,'用户支付成功发送短信失败，订单号为：'.$order_id);
    }
    ...
    public function MssageSendDeliver($order_id)
    {
        ...
        $this->send($switch,$storeInfo->user_phone,compact('order_id','store_name','nickname'),AliSMS::DELIVER_GOODS_CODE,'用户发货发送短信失败，订单号为：'.$order_id);
    }
    ...
    public function MssageSendReceiving($order_id)
    {
        ...
        //模板变量
        $store_name = StoreOrder::getProductTitle($storeInfo->cart_id);
        $this->send($switch,$storeInfo->user_phone,compact('store_name','order_id'),AliSMS::TAKE_DELIVERY_CODE,'用户确认收货发送短信失败，订单号为：'.$order_id);
    }
    ...
    public function MssageSendAdminPlaceAnOrder($order_id)
    {
        ...
        $switch && $this->getAdminNoticeAuth(function ($userInfo) use($storeInfo) {
            ...
            $this->send(true,$userInfo->phone,compact('admin_name','order_id'),AliSMS::ADMIN_PLACE_ORDER_CODE,'用户下单成功管理员发送短信通知失败，订单号为：'.$storeInfo->order_id);
        });
    }
    ...
    public function MssageSendAdminPaySuccess($order_id)
    {
        ...
        $switch && $this->getAdminNoticeAuth(function ($userInfo) use($storeInfo) {
            ...
            $admin_name = $userInfo->nickname;
            $order_id = $storeInfo->order_id;
            $this->send(true,$userInfo->phone,compact('admin_name','order_id'),AliSMS::ADMIN_PAY_SUCCESS_CODE,'用户支付成功管理员发送短信通知失败，订单号为：'.$storeInfo->order_id);
        });
    }
    ...
    public function MssageSendAdminConfirmTakeOver($order_id)
    {
        ...
        $switch && $this->getAdminNoticeAuth(function ($userInfo) use($storeInfo) {
            ...
            $admin_name = $userInfo->nickname;
            $order_id = $storeInfo->order_id;
            $this->send(true,$userInfo->phone,compact('admin_name','order_id'),AliSMS::ADMIN_TAKE_DELIVERY_CODE,'用户确认收货成功管理员发送短信通知失败，订单号为：'.$storeInfo->order_id);
        });
    }
    ...
    public function MssageSendAdminRefund($order_id)
    {
        ...
        $switch && $this->getAdminNoticeAuth(function ($userInfo) use($storeInfo) {
            ...
            $admin_name = $userInfo->nickname;
            $order_id = $storeInfo->order_id;
            $this->send(true,$userInfo->phone,compact('admin_name','order_id'),AliSMS::ADMIN_RETURN_GOODS_CODE,'用户退款管理员发送短信通知失败，订单号为：'.$storeInfo->order_id);
        });
    }
    /**
     * 提取管理员权限
