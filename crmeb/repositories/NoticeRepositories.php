...
class NoticeRepositories
{
    ...
    public static function noticeOrderPaySuccess($order,$formId)
    {
        ...
        $wechatUser = WechatUser::where('uid',$order['uid'])->field('openid','routine_openid')->findOrEmpty();
        ...
        if($switch){
            try{
                ...
                $siteName = sysConfig('site_name');
                $instance->orderPrinting($instance->setContent($siteName,is_object($order) ? $order->toArray() : $order,$product));
            }catch (\Exception $e){
